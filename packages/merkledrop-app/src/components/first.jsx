...
export  default  class LogoGather extends React.Component {
    ...
    render() {
        const {contentShow,styleShow} = this.state;
        return (
            <div className="logo-gather-demo-wrapper">
                <canvas id="canvas" />
                {
                    !contentShow &&
                    <div className='connect' onClick={this.connect}>connect wallet</div>
                }
                {
                    contentShow &&  <div className={styleShow ? 'bgBrdr btm2Top rain':'bgBrdr rain top2Btm'} >
                        <AppInner  ref={this.myRef} onMouseEnter={this.onReset} />
                    </div>
                }
